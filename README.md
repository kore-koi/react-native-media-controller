...
- [`android/`](android): All your `android`-specific implementations.
  - [`build.gradle`](android/build.gradle): The gradle build file. This contains four important pieces:
    ...
    3. Adds all Nitrogen files (`apply from: '.../RNMediaController+autolinking.gradle'`)
  ...
  - [`CMakeLists.txt`](android/CMakeLists.txt): The CMake build file to build C++ code. This contains four important pieces:
    1. Creates a library called `RNMediaController` (same as in `nitro.json`)
    2. Adds all Nitrogen files (`include(.../RNMediaController+autolinking.cmake)`)
  ...
  - [`src/main/java/com/margelo/nitro/mediacontroller/`](android/src/main/java/com/margelo/nitro/mediacontroller/): All Kotlin implementations.
    - [`RNMediaControllerPackage.java`](android/src/main/java/com/margelo/nitro/mediacontroller/RNMediaControllerPackage.java): The react-native package. You need this because the react-native CLI only adds libraries if they have a `*Package.java` file. In here, you can autolink all Kotlin HybridObjects.
